...
float _BlendAmount;
    ...
    inline VertexOutputForwardBaseMOR vOutToRadialVout(VertexOutputForwardBase i, VertexInputMOR v) { //Lame not good function.
...
#endif
{
    ...
    #if _DETAIL_MULX2
        albedo -= lerp( ((1-detailAlbedo)*0.1),1-detailAlbedo,Luminance(col));//LerpWhiteTo (detailAlbedo * unity_ColorSpaceDouble.rgb, mask);
    #elif _DETAIL_MUL
...
#endif
{
    ...
    half3 specColor;
        ...
        float3 col = Albedo_MOR(i_tex,i_color.rgb);
    #endif
    half3 diffColor = DiffuseAndSpecularFromMetallic (col*i_color.rgb, metallic, /*out*/ specColor, /*out*/ oneMinusReflectivity);
...
#else
inline FragmentCommonData FragmentSetup_MOR (inout float4 i_tex, float3 i_eyeVec, half3 i_viewDirForParallax, float4 tangentToWorld[3], float3 i_posWorld,float4 i_color,float screenDistance,float vface)
#endif
{
...
half4 fragForwardBase_MOR (VertexOutputForwardBaseMOR i, UNITY_VPOS_TYPE screenPos : SV_POSITION) : SV_Target
{
    ...
    //return half4(gi.indirect.specular,1);
        ...
        //gi.indirect.specular = gi.indirect.specular;
        #if _LIGHTMAP
        #else
            s.specColor = 0;
        #endif
    ...
    UNITY_EXTRACT_FOG_FROM_EYE_VEC(i);
        ...
        float3 rim = _ColorRimlight * x;//(rimAmount-minus) ;
