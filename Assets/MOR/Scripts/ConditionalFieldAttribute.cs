...
public class ConditionalFieldAttribute : PropertyAttribute
{
    ...
    public ConditionalFieldAttribute(string propertyToCheck, bool readOnly, object compareValue = null)
    {
	    _propertyToCheck = propertyToCheck;
	    _compareValue = compareValue;
	    ReadOnly = readOnly;
    ...
    private SerializedProperty FindRelativeProperty(SerializedProperty property, string toGet)
    {
        ...
        if (nestedProperty == null)
        {
            ...
            var who = string.Format("Property <color=brown>{0}</color> in object <color=brown>{1}</color> caused: ",
                arrayProp.name,
                target.name);
...
public class ConditionalFieldAttributeDrawer : PropertyDrawer
{
    ...
    {
        if (_toShow) {
	        bool enabled = !Attribute.ReadOnly;//GetConditionalHideAttributeResult(condHAtt, property);
