...
public class BrushManifest : ScriptableObject {
  ...
  public Dictionary<Guid, BrushDescriptor> BrushesByGuid {
    get {
      if (m_ByGuid == null) {
        m_ByGuid = new Dictionary<Guid, BrushDescriptor>();
        foreach (BrushDescriptor brush in m_Brushes) {
          if (m_ByGuid.ContainsKey(brush.m_Guid)) {
            //Debug.Log($"Duplicate ID : {brush.name} : existing =  {m_ByGuid[brush.m_Guid].name}");
            //m_ByGuid[brush.m_Guid] = brush;
            continue;
          }
          m_ByGuid.Add( (Guid)brush.m_Guid, (brush));
        }
