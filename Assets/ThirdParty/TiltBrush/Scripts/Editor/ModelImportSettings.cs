...
public class ModelImportSettings : AssetPostprocessor {
  ...
  private bool testedForTiltbrush = false;
  ...
  Material OnAssignMaterialModel(Material material, Renderer renderer) {
    ...
    if ( !testedForTiltbrush ) {
      Debug.Log("TITLBRUSH : Assigning materials with no preprocess?" );
      m_Info = FbxUtils.GetTiltBrushFbxInfo(assetPath);
      testedForTiltbrush = true;
    }
    if (! IsTiltBrush) {
      if ( assetPath.EndsWith("tilt.fbx") ) {
        Debug.LogWarning("TILTBRUSH: Maybe missed tiltbrush material apply?");
      }
      return null;
    }
    ...
    if (desc != null) {
      ...
      Debug.Log($"TILTBRUSH: lookup failure maybe? : {renderer.transform.root.name} | mat = {material.name}");
      return null;
    }
  }
  ...
  BrushDescriptor GetDescriptorForStroke(string oldMaterialName) {
    ...
    if (match.Success) {
      ...
      try {
        ...
        LogWarningWithContext($"Unexpected: Couldn't find Tilt Brush material for guid {brushGuid}.");
        return null;
      }
    }
    ...
    // Older versions use material.name == brush.m_DurableName
    {
      ...
      if (descs.Length == 1) {
        ...
        LogWarningWithContext($"Ambiguous brush name {oldMaterialName}; try exporting with the latest version of Tilt Brush. Brush may have the incorrect material.");
        return descs[0];
      } else {
        LogWarningWithContext($"Unexpected: Couldn't find Tilt Brush material for name {oldMaterialName}");
        return null;
      }
    }
  }
