//#Edited by MOR to allow file identification by extension
  ...
  public static FbxInfo GetTiltBrushFbxInfo(string path, bool force=false) {
    ...
    if (force || path.ToLowerInvariant().EndsWith(".fbx")) {
      ...
      if (propsIter != null) {
        ...
        if (info.tiltBrushVersion == null && path.EndsWith("tilt.fbx")) { //#Ljo : MOR - hack so that tiltbrush can get imported by filename
          info.tiltBrushVersion = new Version(){major = 16};
          info.requiredToolkitVersion = new Version(){major = 10};
        }
      }
    }
    return info;
