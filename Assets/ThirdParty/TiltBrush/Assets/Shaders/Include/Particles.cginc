...
float4 OrientParticle(float3 center, float halfSize, int corner, float rotation) {
  ...
  float3 up, rt; {
    ...
    float scale = 1;
  #else
    float scale = length(unity_ObjectToWorld[0].xyz);
  #endif
  return float4(_RotatedQuadCorner(up, rt, center, halfSize  * scale, corner, rotation).xyz, 1);//MOR : LJO - adjust for scaling
}
