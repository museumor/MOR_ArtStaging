...
Properties {
  ...
  _Size("Size", Float) = 1
  [Toggle(_IGNORE_SCALE)]_IgnoreScale ("Ignore Scale",int) = 0
  _ShrinkStart ("ShrinkStartDistance",float) = 0
  _ShrinkEnd ("ShrinkEndDistance",float) = 0
  _MinSize ("minimum size",float) = 0.3
}
...
Category {
  ...
  SubShader {
    Pass {
      ...
      #pragma shader_feature_local _ _IGNORE_SCALE 
      // MOR : ignore scale option 
      ...
      v2f vert (ParticleVertex_t v)
      {
        ...
        float halfSize = GetParticleHalfSize(v.corner.xyz, v.center, birthTime) * _Size;
        float dist = length(v.center.xyz-mul(unity_WorldToObject, float4(_WorldSpaceCameraPos.xyz, 1)).xyz);
        //dist = length(ObjSpaceViewDir(v.center.xyz));
        halfSize = lerp(_MinSize*halfSize,halfSize,saturate((dist-_ShrinkStart)/(1+_ShrinkEnd)));
        halfSize *=0.5;
      ...
      float4 frag (v2f i) : SV_Target
      {
        ...
        c.a = 1;
        c = SrgbToNative(c);
        return float4(pow(c.rgb,40), 1.0);
      }
      ENDCG
    }
  }
