...
_ClipDistanceStart("Clip Distance Start",float) =0.1
_FarClip("Far Clip",float)=1000000.0
[Toggle(_USEDISTANCECLIP)]_DistanceClip("DistancClip",int) = 0
[Toggle(_SCALECLAMP)]_ScaleClamp("ScaleClamp",int) = 0

_ClampFactor ("clampSize",float) = 1
}
  ...
  SubShader {
    Pass {
      ...
      #pragma multi_compile_particles
      		...
      		#pragma shader_feature_local _ _USEDISTANCECLIP
      		#pragma shader_feature_local _ _SCALECLAMP
      ...
      float4 dist_from_plane(float3 plane_normal, float3 point_on_plane, float3 pos) {
        ...
        float _ClampFactor;
        float _FarClip;
        float _ClipDistanceStart;
      v2f vert (ParticleVertex_t v, out float4 vertex : SV_POSITION )
      {
        ...
        #if _SCALECLAMP
            halfSize = min(halfSize,_ClampFactor);
        #endif
        ...
        //float3 disp = center.rgb;
        center_WS.xyz += mul(xf_CS, float4(disp, 0));
        float distanceFromEye = length(center_WS - _WorldSpaceCameraPos);
        
        //halfSize = lerp(0,halfSize,saturate((distanceFromEye-_FadeDistanceStart) * (100/_FadeDistanceEnd)));
        float4 corner = OrientParticle_WS(center_WS.xyz, halfSize, v.vid, rotation);
        vertex = mul(UNITY_MATRIX_VP, corner);
        #if _USEDISTANCECLIP
            vertex = lerp(vertex,float4(0.5,-2,-1,0),(distanceFromEye < (_ClipDistanceStart)) || distanceFromEye > _FarClip);
      ...
      float screenDistance =  length(screenPos.z/screenPos.w);	//this is a 0-1 value of screen depth.
        ...
        c = SrgbToNative(c);
        c.a *= _AlphaMultiplier*distanceLerp;
        return c;
      }
      ENDCG
    }
