...
[HDR]_Color("Color",Color) = (1,1,1,1)
  ...
  [PerRendererData]_MORBloom ("Bloom for the MOR",float) = 1
}
...
Category {
  ...
  SubShader {
    Pass {
      ...
      float4 _MainTex_ST;
      float _EmissionGain;
      float _MORBloom = 1;//MOR added blooom : LJO
      struct appdata_t {
      ...
      float4 frag (v2f i) : SV_Target
      {
        ...
        return float4(pow(color.rgb,_MORBloom)*_Color.rgb, 1.0);//MOR added blooom : LJO
      }
